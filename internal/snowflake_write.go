...
func (sf *SfDB) loadStage(ctx context.Context, stage string, loadTime int64, datasetDefinition *common.DatasetDefinition) error {
	...
	USING (
		...
		%s
		FROM (SELECT $1, ROW_NUMBER() OVER(ORDER BY null) as ix FROM @%s)
        QUALIFY ROW_NUMBER() OVER (PARTITION BY id ORDER BY recorded DESC, ix DESC) = 1
	) AS src
	ON latest.id = src.id
	WHEN MATCHED THEN
		UPDATE SET
...
func (sf *SfDB) loadFilesInStage(ctx context.Context, files []string, stage string, loadTime int64, datasetDefinition *common.DatasetDefinition) error {
	...
	USING (
		...
		'%s'::varchar as dataset,
		%s
		FROM (SELECT $1, ROW_NUMBER() OVER(ORDER BY null) as ix FROM @%s (PATTERN => '.*(%s)'))
        QUALIFY ROW_NUMBER() OVER (PARTITION BY id ORDER BY recorded DESC, ix DESC) = 1
	) AS src
	ON latest.id = src.id
	WHEN MATCHED THEN
